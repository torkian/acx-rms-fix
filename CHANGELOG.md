...
### Added
- **`--dry-run` CLI flag** — prints what the tool *would* do for each input file (action, input path, and computed output path) without calling ffmpeg or writing any files. Always exits 0. Useful for previewing batch jobs before committing: `acx-rms-fix --dry-run -o mastered/ *.mp3`.
