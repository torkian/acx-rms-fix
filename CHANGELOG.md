...
- **`--dry-run` CLI flag** — previews what would be done for each input file (fix, check, or replace, including the computed output path) without calling ffmpeg at all. Useful for verifying glob expansion and output-path logic before committing a long batch run. Combine with `-o`, `--replace`, or `--check` to preview that mode. Exits 0 if all inputs exist, 2 if any are missing.
